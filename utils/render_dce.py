...
# # Load SMPL model
m = load_model('/data/dataset/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl')


face_colors = np.zeros((len(m.f), 3), dtype=np.float32)
vertex_colors = np.load('/data/dataset/DCEReID/vert_colors_3D.npy')

vertex_colors = vertex_colors[0:6890].round().astype(np.uint8)
vertex_colors_float = vertex_colors.astype(np.float32) / 255.0  # 转成 0~1

for i in range(len(m.f)):
    v0, v1, v2 = m.f[i]
    c0 = vertex_colors_float[v0]
    c1 = vertex_colors_float[v1]
    c2 = vertex_colors_float[v2]
    face_colors[i] = (c0 + c1 + c2) / 3.0
...
for i in tqdm(range(len(m.f))):
    ...
    # draw part triangle
    ax.plot_trisurf(x_list, y_list, z_list, color=face_colors[i], edgecolor='black', linewidth=0.15)
    # draw lines
    # x_list.append(float(m[vertex_lst[0]][0])
    # y_list.append(float(m[vertex_lst[0]][1]))
    # z_list.append(float(m[vertex_lst[0]][2]))
