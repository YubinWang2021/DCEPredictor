...
from utils.dataset_utils import DCE_ANN_KEYS, DCE_IUV_KEYS, init_from_coco_json_file
from utils.dataset_utils import extract_segmentation_mask
...
class DCEDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img_save = ret['img'].clone()
        #img_save[ret['dp_y'].round().long(), ret['dp_x'].round().long()] = torch.Tensor([0, 0, 255]).cuda().unsqueeze(
        #    1)
        #cv2.imwrite('/media/data3/wangyubin/train_img_{}'.format(random.randint(0, 10000)), img_save)
...
if __name__ == '__main__':
    test = ['/data/dataset/coco2014/DP3D_train.json',
            '/data/dataset/coco2014/DP3D_query.json',
            '/data/dataset/coco2014/DP3D_test.json']
    dataset = DCEDataset(test)
    for i in range(1000):
        img = dataset[i]
