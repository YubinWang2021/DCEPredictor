...
import ctypes
from model.dce_model import DCEModel
...
from dataset.dce_dataset import DCEDataset
...
def get_args_parser():
    ...
    parser.add_argument('--blr', default=1.5e-4, type=int)
...
def configure_file_list(args):
    train_json_list = [
        '/data/dataset/coco2014/densepose_coco_2014_train.json',
        '/data/dataset/coco2014/densepose_coco_2014_valminusminival.json'
    ]
    test_json_list = [
        '/data/dataset/coco2014/densepose_coco_2014_minival.json'
    ]
    args.dataset_weight = [5, 3]
    args.dataset_weight = [x / sum(args.dataset_weight) for x in args.dataset_weight]
...
def main(args):
    ...
    args.lr = args.blr * args.batch_size
    model = timm.create_model('dce_darknet19_binary2')
    model = model.cuda()
    if args.finetune is not None:
        checkpoint = torch.load(args.finetune, map_location='cpu')
        print('Load pre-trained checkpoint from: %s' % args.finetune)
        for k in ['model', 'module', 'encoder']:
            if k in checkpoint:
    ...
    train_set = DCEDataset(train_json_list, train=True, repeat=args.repeat)
    print(test_json_list)
    test_sets = [DCEDataset(test_json_list)]
    ...
    test_loaders = [DataLoader(
        test_set, batch_size=4, num_workers=args.num_workers, collate_fn=collate_fn, pin_memory=True, shuffle=False,
    ...
    if args.eval:
        test(model, test_loaders, test_json_list, args.ckpt_dir, args.local_rank, args)
        exit(0)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if ((epoch + 1) % args.save_checkpoint_each_epoch == 0 or epoch + 1 == args.max_epochs):
            ...
            test(model, test_loaders, test_json_list, args.ckpt_dir, args)
