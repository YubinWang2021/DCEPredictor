...
import argparse
import json
from evaluater.dce_evaluater import test
import utils.lr_sched as lr_sched
import timm.optim.optim_factory as optim_factory
import ctypes
import model.dce_model
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='./DP3D', type=str)
    # others
...
def train_epoch(model, optimizer, train_loader, loss_scaler, epoch, args):
    ...
    for data_iter_step, samples in enumerate(train_loader):
        ...
        loss_dce = losses['loss_dce'].item()
        #loss_dp_masks = losses['loss_dp_masks'].item()
        loss = sum(losses.values())
        loss_value = loss.item()
        total_loss += loss_value
        total_dce_loss += loss_dce
        ...
        summary_string = f'({data_iter_step + 1}/{len(train_loader)}) | Total: {bar.elapsed_td} | ' \
                         ...
                         f'loss_dce: {total_dce_loss / (data_iter_step + 1):.4f} | ' \
                         f'|acc: {acc:.4f} | lr: {lr}'
...
def main(args):
    ...
    print(args.lr)
    model = timm.create_model('dce_effunet64')
    model = model.cuda()
    ...
    test_sets = [DCEDataset([x]) for x in test_json_list]
